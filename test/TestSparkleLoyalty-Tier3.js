 ...
 */
contract('SparkleLoyalty - Test coverage: Tier3(30%) Workflow', async accounts => {
  ...
  it('Initialize contract(s)', async() => {
    ...
    /// Configure controller contract address
    await tsi.setContractAddress(pol.address, {from: OWNER});
    /// Configure reward time period to be 86400 seconds (24H~)
   ...
   */
  describe('Step2: Deposit 1,000 tokens to SparkleLoyalty by USER1', async () => {
    ...
    /**
     * @dev Test purchase of tier3 reward boost(30%)
     * @notice Sent: 0.20 eth Expected: 0.30 eth
     */
    it('selectRewardTier(3, { from: USER1, value: tier2eth }) should fail  failed', async () => {
      /// Attempt to select a new reward tier
      await pol.selectRewardTier(3, {from: USER1, value: tier2eth})
      .then((tx) => {
    ...
    /**
     * @dev Test purchase of tier3 reward boost(30%)
     * @notice Sent: 0.30 eth Expected: 0.30 eth
     */
    it('Increase reward tier to Tier3 (30%) (Sent: 0.30eth) passed', async () => {
      /// Attempt to select tier3 reward boost(30%)
      await pol.selectRewardTier(3, {from: USER1, value: tier3eth})
      .then((tx) => {
        truffleAssert.eventEmitted(tx, 'TierSelectedEvent', (event) => {
          /// Return success if expected values returned
          return (event[1].eq(new BN(3)));
        })
      })
      .catch((error) => {
        /// Should not get here, return failure
    ...
    /**
     * @dev Test purchase of tier2 reward boost(20%)
     */
    it('selectRewardTier(2, { from: USER1, value: tier2eth }) should fail', async () => {
      await pol.selectRewardTier(2, { from: USER1, value: tier2eth })
      .then((tx) => {
        /// Should not get here, return failure
        return assert(false);
      })
  ...
  describe('Step3: Claim loyalty Day 1', async () => {
     ...
     */
    it('Collected reward(s) should equal 0.17797 tokens', async () => {
      /// Return success if expected values are returned
      assert.equal(await pol.getTokensCollected(USER1) / 10e7, 0.17797);
    });
  ...
  describe('Step3: Claim loyalty Day 2', async () => {
     ...
     */
    it('Collected reward(s) should equal 0.35597167 tokens', async () => {
      /// Return success if expected values are returned
      assert.equal(await pol.getTokensCollected(USER1) / 10e7, 0.35597167);
    });
  ...
  describe('Step3: Claim loyalty Day 3', async () => {
     ...
     */
    it('Collected reward(s) should equal 0.53400502 tokens', async () => {
      /// Return success if expected values are returned
      assert.equal(await pol.getTokensCollected(USER1) / 10e7, 0.53400502);
    });
  ...
  describe('Step3: Claim loyalty Day 4', async () => {
    ...
    it('Loyalty claim @ ~12h should fail', async () => {
      ...
      /// Attempt to claim loyalty rewards for USER1
      await pol.claimLoyaltyReward({ from: USER1 })
      .catch((err) => { assert.equal(err.reason, 'Not mature'); });
    });
     ...
     */
    it('Collected reward(s) should equal 0.71207005 tokens', async () => {
      /// Return success if expected values are returned
      assert.equal(await pol.getTokensCollected(USER1) / 10e7, 0.71207005);
    });
  ...
  describe('Step3: Claim loyalty Day 5', async () => {
     ...
     */
    it('Collected reward(s) should equal 0.89016677 tokens', async () => {
      /// Return success if expected values are returned
      assert.equal(await pol.getTokensCollected(USER1) / 10e7, 0.89016677);
    });
  ...
  describe('Step3: Claim loyalty Day 365 (Days 6-364 skipped)', async () => {
     ...
     */
    it('Collected reward(s) should equal 65.01639904 tokens', async () => {
      /// Return success if expected values are returned
      assert.equal(await pol.getTokensCollected(USER1) / 10e7, 65.01639904);
    });
  ...
  describe('Step4: Withdraw loyalty', async () => {
     ...
     */
    it('Currrent collected rewards should equal 65.01639904 tokens', async () => {
      /// Return success if expected values are returned
      assert.equal(await pol.getTokensCollected(USER1) / 10e7, 65.01639904);
    });
     ...
     */
    it('Total withdraw amount should equal 1065.01639904 tokens', async () => {
      /// Return success if expected values are returned
      assert.equal(await pol.getTotalBalance(USER1) / 10e7, 1065.01639904);
    });
     ...
     */
    it('getRewardTier(USER1) should equal Tier3', async () => {
      /// Attempt to obtain USER1's tier reward index
      await pol.getRewardTier(USER1)
      .then((response) => {
        /// Return success is expected value returned
        return assert.equal(response, 3);
      })
      .catch((error) => {
        /// Should never get here, return failure
        return assert(false);
    ...
    it('lockAccount(USER1, true, { from: OWNER }) should pass', async () => {
      ...
      await pol.lockAccount(USER1, true, { from: OWNER })
      .then((response) => {
    ...
    it('lockAccount(USER1, false, { from: OWNER }) should pass', async () => {
      ...
      await pol.lockAccount(USER1, false, { from: OWNER })
      .then((response) => {
        truffleAssert.eventEmitted(response, 'LockedAccountEvent', (event) => {
          /// Return success is expected value returned
  ...
  describe('Post: Audit claimed and token totals', async () => {
     ...
     */
    it('getTotalTokensClaimed() should return 65.01639904', async () => {
      ...
      .then((count) => {
        ...
        return assert.equal(count, 6501639904);
      })
      .catch((error) => {
        /// Should not get here, return failure
        return assert(false);
  ...
  describe('Finalize: Return any tokens deposited', async() => {
    ...
    it('Return tokens to Accounts[2]', async() => {
      ...
      await st.methods.transfer(TREASURY, 65.01639904 * 10e7).send({ from: USER1 });
    });
