...
contract('SparkleLoyalty - Workflow @ Tier1(10%)', async accounts => {
  ...
  describe('Step4: Withdraw loyalty', async() => {
    ...
    it('Currrent collected rewards should equal 56.49963222 tokens', async () => {
      assert.equal(await pol.getTokensCollected(USER1) / 10e7, 56.49963222);
    });

    it('Total withdraw amount should equal 1056.49963222 tokens', async () => {
      assert.equal(await pol.getTotalBalance(USER1) / 10e7, 1056.49963222);
    });
  ...
  describe('Finalize: Return any tokens deposited', async() => {
    it('Return tokens to Accounts[2]', async() => {
      await st.methods.transfer(accounts[1], 56.49963222 * 10e7).send({
        from: accounts[2]
      });
    });
  });
