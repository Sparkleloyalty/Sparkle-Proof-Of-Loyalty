...
contract('SparkleLoyalty - Workflow @ Tier3(30%)', async accounts => {
  let pol, st, ts, rt;
  ...
  describe('Step2: Deposit Tokens', async () => {
    ...
    it('Increase reward tier to Tier3 (30%) (Eth: 0.30) passed', async () => {
      truffleAssert.eventEmitted(await pol.selectRewardTier(3, { from: USER1, value: tier3eth }), 'TierSelectedEvent');
    });
  });
  ...
  describe('Step4: Withdraw loyalty', async() => {
    ...
    it('Currrent collected rewards should equal 67.10915564 tokens', async () => {
      assert.equal(await pol.getTokensCollected(USER1) / 10e7, 67.10915564);
    });

    it('Total withdraw amount should equal 1067.10915564 tokens', async () => {
      assert.equal(await pol.getTotalBalance(USER1) / 10e7, 1067.10915564);
    });
  ...
  describe('Finalize: Return any tokens deposited', async() => {
    it('Return tokens to Accounts[2]', async() => {
      await st.methods.transfer(accounts[1], 67.10915564 * 10e7).send({
        from: accounts[2]
      });
    });
  });
