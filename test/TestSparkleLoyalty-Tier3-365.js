...
contract('SparkleLoyalty - Workflow @ Tier0(5%)', async accounts => {
  ...
  describe('Step4: Withdraw loyalty', async() => {
    ...
    it('Currrent collected rewards should equal 349.42423202 tokens', async () => {
      assert.equal(await pol.getTokensCollected(USER1) / 10e7, 349.42423202);
    });

    it('Total withdraw amount should equal 1349.42423202 tokens', async () => {
      assert.equal(await pol.getTotalBalance(USER1) / 10e7, 1349.42423202);
    });
  ...
  describe('Finalize: Return any tokens deposited', async() => {
    it('Return tokens to Accounts[2]', async() => {
      await st.methods.transfer(accounts[1], 349.42423202 * 10e7).send({
        from: accounts[2]
      });
    });
  });
